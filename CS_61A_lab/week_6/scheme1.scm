...
(define (eval-1 exp)
  (cond ((constant? exp) exp)
        ...
        ((quote-exp? exp) (cadr exp)) ; Here read will transform '() to (quote ()).
        ((if-exp? exp)
         (if (eval-1 (cadr exp))
           (eval-1 (caddr exp))
           (eval-1 (cadddr exp))))
        ...
        ((define-exp? exp)
         ...
         ;; IMHO this means we can't define named-lambda function 
...
(define (apply-1 proc args)
  ;; > you have all the Scheme primitives for arithmetic and list manipulation
...
((lambda (f n)
 ...
 (lambda (x) (first (string->list (symbol->string x)))) ; alternative for the notes' assumed first to get the first character.
 '(the rain in spain))
; (t r i s)
