        ...
        (if (all-vars-has-bindings operand-vars frame)
          ...
          (begin
            ...
            (singleton-stream 
              ...
              (append frame (list (cons operand-vars (delay orig-proc))))
            ))
          )))
   frame-stream))
...
(define (try-force-promise frame)
  (let ((pair-with-promise (find-pair-with-promise frame)))
    (if (and pair-with-promise (all-vars-has-bindings (caar pair-with-promise) frame))
      ...
      (begin
        ...
        (stream-flatmap
          try-force-promise
          ;; return frame-stream
          ((force (cdar pair-with-promise)) frame)))
      (singleton-stream frame)
      )))
...
;; See exercise_codes/SICP/4/test/deep-copy-test.scm. This won't work for improper list.
...
(define (use-separate-delay frame)
  (map 
    (lambda (binding) 
      (let ((promise (cdr binding)))
  ...
  (stream-flatmap
   (lambda (frame)
    ...
    (stream-flatmap ; keep the order 
      (lambda (frame)
        (try-force-promise frame)
        )
      (stream-append-delayed
        (find-assertions query-pattern (use-separate-delay frame))
        (delay (apply-rules query-pattern (use-separate-delay frame)))))
    )
   frame-stream))
