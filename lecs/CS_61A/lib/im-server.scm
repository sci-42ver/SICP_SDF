...
(load "im-common.scm")
;; UCB STK primitive
(define listened-ports-table (make-hash-table))

(define mutex (make-mutex))

(define (when-port-readable port handler)
  ...
  (if handler
    (let ((listener (make-thread (make-listener-with handler))))
      ...
      (print "Starting thread: " listener)
      (thread-start! listener))
    (hash-table-set! listened-ports-table port #f)))

; UCB STK Primitive

(define listened-sockets-table (make-hash-table))

(define (when-socket-ready socket handler)
  (define (request-handler socket)
    (let ((callback (make-thread (handler socket))))
      (thread-start! callback)))
  (define (wait-for-request)
    (let ((waiting (make-thread (lambda ()
                                  (let loop ()
                                    ;;Accept the socket connection
                                    (let ((new-socket (socket-accept socket)))
                                      (print "Got request, calling request handler")
                                      (if (hash-table-ref listened-sockets-table socket)
                                        (begin (request-handler new-socket)
                                               (loop)))))))))
      (hash-table-set! listened-sockets-table socket #t)
      (thread-start! waiting)))
  (if handler
    (wait-for-request)
    (hash-table-set! listened-sockets-table socket #f)))
...
(define (im-server-start)
  ...
  (when-socket-ready server-socket (lambda (socket)
                                     (lambda ()
                                       (begin
