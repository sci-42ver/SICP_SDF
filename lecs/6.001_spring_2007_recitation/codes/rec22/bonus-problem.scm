...
(define (m-eval exp env)
  (cond ((self-evaluating? exp) exp)
        ...
        ;; 1. just give the constructor and selector.
        ;; 2. > change 3 lines of code and add about 10 new lines
        ;; Here no helper procedures are defined. so only 2.
        ;; 2.1. Same as eval-cons-stream in sol.
        ((tagged-list? exp 'cons-stream) (cons (m-eval (cadr exp) env) (delay-it (caddr exp) env))) ; see lecs/6.001_spring_2007_recitation/codes/rec22/source22/streams.scm
        ;; sol this is implied by cdr and then force-it when output.
        ((tagged-list? exp 'stream-cdr) (force-it (cdr (cadr exp))))
        ((application? exp)
         ;; see sol. Besides the interface, we need to change the rest for compatibility like what exercise_codes/SICP/4/Lazy_Evaluation_lib.scm does.
...
;; thunk from exercise_codes/SICP/4/Lazy_Evaluation_lib.scm
